  ...
  <span className="block sm:inline text-green-500"> Committee PCE</span>
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-10">
        {[
          { value: 950, suffix: "+", desc: "Students Placed" },
          { value: 120, suffix: "+", desc: "Recruiting Companies" },
          { value: 18, suffix: " LPA", desc: "Highest Package" },
          { value: 92, suffix: "%", desc: "Placement Rate" },
        ].map((item, i) => (
          <motion.div
            key={i}
         className="bg-gradient-to-br from-gray-900/30 to-gray-800/30 p-10 shadow-lg rounded-lg backdrop-blur-sm border border-gray-800 opacity-90"
          ...
          >
            ...
            <p className="text-sm mt-2 text-white">{item.desc}</p>
          </motion.div>
        ))}
      </div>
    </motion.div>
    ...
    {[...Array(2)].map((_, i) => (
      <div className="flex space-x-10" key={i}>
        {recruiters.map((company) => (
          <div
            key={`${company.id}-${i}`}
            className="w-44 h-32 bg-white rounded-md shadow-md flex items-center justify-center"
          >
            <img
              src={company.logo}
      ...
      {(() => {
        ...
        return (
          <>
            ...
            <div className="block md:hidden relative w-full max-w-sm mx-auto overflow-hidden min-h-[380px]">
              <div
